  ...
  displayFn: ({ field, row }) => (
    <div>
      <div>
        ...
        <img src={field.display} style={{ height: 30, borderRadius: 500 }} role="presentation" />
  ...
  displayFn: ({ row }) => {
    ...
    for (let i = 0; i < 6; i++) {
      if (row[`item_${i}`].display) {
        itemArray.push(<img style={{ height: 25, margin: '0 3px', borderRadius: 10 }} role="presentation" src={row[`item_${i}`].display} />);
      }
    }
    return itemArray;
  },
