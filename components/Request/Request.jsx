...
class Request extends React.Component {
  ...
  componentDidMount() {
    this.setState(Object.assign({}, this.state, {match_id: window.location.hash.slice(1)}));
  }
  handleChange(event) {
  ...
  handleSubmit() {
    ...
    fetch(`${API_HOST}/api/request_job`, {
      ...
      body: JSON.stringify({match_id: this.state.match_id, replay_blob: this.state.replay_blob}),
    }).then(jsonResponse).then(function(json){
      if (json.error) {
    ...
    {
      fetch(`${API_HOST}/api/request_job?id=${job_id}`).then(jsonResponse).then(function(json){
