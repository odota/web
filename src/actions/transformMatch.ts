...
function generateVisionLog(match: Match) {
  const computeWardData = (player: MatchPlayer, i: number) => {
  ...
  const temp: any[] = (match.players || []).map(computeWardData).flat();
...
function transformMatch(m: any) {
  ...
  // lane winning
  const lineResults = m.players.reduce((res: any[], pl: any) => {
  ...
  const newObjectives = (m.objectives || []).map((obj: any) => {
    if (obj.slot > 0) {
      return {
        ...obj,
