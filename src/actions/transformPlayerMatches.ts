...
export default function transformPlayerMatches(fields: any) {
  return (response: any[]) => {
    ...
    if (
      fields.included_account_id &&
      !Array.isArray(fields.included_account_id)
    ) {
      return response.map((match) => {
        let sameTeam = false;
        const found = Object.entries(match.heroes).find(
          ([key, val]: [key: string, val: any]) =>
            val.account_id &&
            val.account_id.toString() === fields.included_account_id,
        );
        //@ts-expect-error
