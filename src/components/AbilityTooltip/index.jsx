...
const HeaderBgImg = styled.div`
    ...
    height: 100%;
    width: 20%;
    background: ${({ img }) => `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('${import.meta.env.VITE_IMAGE_CDN}${img}')`};
...
const AbilityTooltip = ({ ability, inflictor }) => (
  ...
  <Wrapper>
    <Header>
      ...
      <HeaderContent>
        {inflictor && inflictor.startsWith('special_') ?
          ...
          <img id="ability-img" src={`${import.meta.env.VITE_IMAGE_CDN}${ability.img}`} alt="Talent Tree" />
            }
    ...
    <Resources>
        {ability.mc &&
        <span>
          <ResourceIcon src={`${import.meta.env.VITE_IMAGE_CDN}/apps/dota2/images/tooltips/mana.png`} alt="Mana" />
          <span className="values">{formatValues(ability.mc)}</span>
        </span>
        }
        {ability.cd &&
        <span>
          <ResourceIcon src={`${import.meta.env.VITE_IMAGE_CDN}/apps/dota2/images/tooltips/cooldown.png`} alt="Cooldown" />
