import { heroes as heroData } from 'dotaconstants';
...
const getFields = (
  ...
  const singleFields = [
    ...
    },
    {
      ...
      value:
        'round((0.3 * kills + (3 - 0.3 * deaths) + 0.003 * (last_hits + denies) + 0.002 * gold_per_min + towers_killed + roshans_killed + 3 * teamfight_participation + 0.5 * observers_placed + 0.5 * camps_stacked + 0.25 * rune_pickups + 4 * firstblood_claimed + 0.05 * stuns)::numeric, 1)',
      key: 'fantasy_points',
      bucket: 1,
    },
    {
    ...
    },
    {
      ...
      value:
        '(SELECT SUM(value::text::int) FROM json_each(damage_inflictor_received))',
      key: 'dmg_received',
    },
  ].map((select) => ({
    ...select,
...
${props.player && props.player.value ? '' : 'AND player_matches.account_id < player_matches2.account_id'}`,
    ...
    order: [
      { text: strings.explorer_asc, value: 'ASC', key: 'asc' },
      { text: strings.explorer_desc, value: 'DESC', key: 'desc' },
    ],
    having,
    limit,
    laneRole: Object.keys(strings)
