...
const autocomplete = (cols: any[], proPlayers: any[] = [], teams: any[] = [], leagues: any[] = []) => {
  ...
  const filteredPros = proPlayers
    ...
    .map((p) => ({
      value: p.name,
      snippet: p.account_id.toString(),
      meta: `${p.account_id.toString()} (${strings.explorer_player})`,
    }));
      ...
      callback(
        ...
        ([] as any[])
          ...
          .concat(
            sqlfuncs.map((e) => ({
              value: e,
              meta: strings.explorer_postgresql_function,
            })),
          )
          ...
          .concat(
            tables.map((e) => ({ value: e, meta: strings.explorer_table })),
          )
          .concat(
            cols.map((e) => ({
              value: `${e.column_name}`,
              meta: `${e.table_name} - ${e.data_type}`,
            })),
          ),
      );
    },
