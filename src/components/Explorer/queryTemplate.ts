...
const tiTeams = [
  7119388, 2586976, 6209166, 8254400, 7391077, 7732977, 8260983, 8291895,
  8599101, 350190, 39, 7390454, 8131728, 8605863, 8721219, 6209804, 8597976,
  2163, 1838315, 15,
];
...
const queryTemplate = (props: Record<string, any[]>) => {
  ...
  selectArray.forEach((x, index) => {
    if (x && x.groupValue) {
      const a = {
        value: x.groupKey,
        groupKeySelect: x.groupKeySelect,
        alias: x.alias,
  ...
  if (
    ...
    selectArray.some((p) => p.template === 'picks_bans')
  ) {
    query = `SELECT
hero_id, 
count(1) total,
...
ORDER BY total ${order?.[0]?.value ?? 'DESC'}`;
    ...
    if (validateArray(groupArray)) {
      groupArray.forEach((x) => {
        groupVal[x.key] =
          `${x.value}${x.bucket ? ` / ${x.bucket} * ${x.bucket}` : ''}`;
      });
    }
    if (validateArray(selectArray)) {
      selectArray.forEach((x) => {
        selectVal[x.key] = x.groupValue || (x && x.value) || 1;
      });
    }
    query = `SELECT
    ...
    ? (validateArray(selectArray) &&
        selectArray.map((x) =>
          [
            (x && x.countValue) || '',
            x && x.avgPerMatch
              ? `sum(${selectVal[x.key]})::numeric/count(distinct matches.match_id) "AVG ${x.text}"`
              : `${x && x.avg ? x.avg : `avg(${selectVal[x.key]})`} "AVG ${x.text}"`,
...
${
  !validateArray(groupArray)
    ? [
        validateArray(selectArray)
          ? selectArray.map((x) => `${x.value} ${x.alias || ''}`)
          : '',
        'matches.match_id',
        'matches.start_time',
...
${
  ...
  (groupArray !== null &&
    groupArray.length > 0 &&
    groupArray[0] &&
    groupArray.some((x) => x.key === 'organization'))
