...
const timingSelect = (itemKey) => ({
  text: `Timing - ${itemData[itemKey].dname}`,
...
const fields = {
  ...
  }, killSelect({ text: strings.heading_courier, key: 'npc_dota_courier'}),
  killSelect({ text: strings.heading_roshan, key: 'npc_dota_roshan'}),
  killSelect({ text: strings.heading_tower, key: '%tower%'}),
  killSelect({ text: strings.heading_barracks, key: '%rax%'}),
  killSelect({ text: strings.heading_shrine, key: '%healers%'})].concat(Object.keys(itemData).filter(itemKey => itemData[itemKey].cost > 2000).map(timingSelect)),
  group: [
    hero,
    player,
    league,
...
class Explorer extends React.Component {
    ...
    componentDidMount() {
      ...
      // Hydrate the state from url
      Object.keys(querystring.parse(window.location.search.substring(1)));
      }
      getQueryString() {
        return `?builder=${encodeURIComponent(JSON.stringify(this.state.builder))}&sql=${encodeURIComponent(this.editor.getSelectedText() || this.editor.getValue())}`;
      }
      instantiateEditor() {
      ...
      render() {
        ...
        proPlayers.forEach(player => proPlayerMapping[player.text] = player.value);
        return (<div>
      ...
      */}
      <div>
        ...
        <ExplorerFormField label={strings.explorer_player_purchased} dataSource={fields.playerPurchased} builderField="playerPurchased" builderContext={this} />
