...
const PlayerSpan = (player?: MatchPlayer) => {
  ...
  const heroName = heroes[player.hero_id]
    ? heroes[player.hero_id].localized_name
    : strings.story_invalid_hero;
  return (
    <span>
      <Tooltip
        title={
      ...
      >
        <StyledStorySpan
          ...
          style={{
            color: player.isRadiant ? constants.colorGreen : constants.colorRed,
          }}
        >
...
const formatList = (list: any[], noneValue: any = []): string[] => {
  ...
  switch (list.length) {
    ...
    case 3:
      return formatTemplate(strings.story_list_3, {
        1: list[0],
        2: list[1],
        3: list[2],
      });
    default:
      return formatTemplate(strings.story_list_n, {
        i: list.shift(),
        rest: formatList(list),
      });
  }
};
...
class RoshanEvent extends StoryEvent {
  ...
  format() {
    ...
    return this.aegis
      ? formatList([formatted, this.aegis.format()])
      : formatted;
  }
}
...
class CourierKillEvent extends StoryEvent {
  ...
  constructor(match: Match, obj: any) {
    ...
    this.killer =
      match.players.find((player) => player.player_slot === obj.killer) || -1;
...
class PredictionEvent extends StoryEvent {
  ...
  constructor(match: Match, team: number) {
    ...
    if (team === -89) {
      ...
      this.players = match.players.filter(
        (player) => player.isRadiant && player.pred_vict,
      );
    } else {
  ...
  format() {
    ...
    return formatTemplate(strings.story_predicted_victory, {
      players: formatList(
        this.players.map(PlayerSpan),
        strings.story_predicted_victory_empty,
      ),
      team: TeamSpan(this.team),
    });
  }
...
class LaneStory {
  ...
  constructor(match: Match, lane: string) {
    ...
    this.dire_players = match.players.filter(
      (player) =>
        player.lane === parseInt(lane, 10) &&
        !player.isRadiant &&
        !player.is_roaming,
    ...
    this.lane = lane;
    this.winning_team =
      getLaneScore(this.radiant_players) > getLaneScore(this.dire_players);
    this.is_draw =
      Math.abs(
        getLaneScore(this.radiant_players) - getLaneScore(this.dire_players),
      ) <= laneScoreDraw;
  ...
  format() {
    ...
    if (this.is_draw) {
      return renderSentence(strings.story_lane_draw, {
        radiant_players: formatList(
          this.radiant_players.map(PlayerSpan),
          strings.story_lane_empty,
        ),
        dire_players: formatList(
    ...
    return renderSentence(
      this.winning_team
        ? strings.story_lane_radiant_win
        : strings.story_lane_radiant_lose,
      {
        radiant_players: formatList(
          this.radiant_players.map(PlayerSpan),
          strings.story_lane_empty,
...
class LanesEvent extends StoryEvent {
  ...
  format() {
    ...
    return [
      ...
      <ul key="lanestory">
        {this.lanes.map((lane) => (
          <li key={lane.lane}>{lane.format()}</li>
        ))}
      </ul>,
    ];
  }
...
const formatBuilding = (event: any) => {
  ...
  if (event.player) {
    template =
      event.player.isRadiant === event.team
        ? strings.story_building_deny_player
        : strings.story_building_destroy_player;
  }
  ...
  format() {
    ...
    [TEAM.radiant, TEAM.dire].forEach((team) => {
      ...
      if (towers.length === 1) {
        ...
        buildingList.push(
          formatTemplate(strings.story_towers_n, {
            team: TeamSpan(team),
            n: towers.length,
          }),
        );
      ...
      Object.keys(localizedLane(strings)).forEach((lane) => {
        const barracks = this.buildings.filter(
          (building) =>
            building.team === team &&
            building instanceof BarracksEvent &&
            building.lane === parseInt(lane, 10),
        );
    ...
    return formatTemplate(strings.story_building_list_destroy, {
      buildings: formatList(buildingList),
    });
  }
}
...
class TeamfightEvent extends StoryEvent {
  ...
  constructor(match: Match, fight: Match["teamfights"][number]) {
    ...
    this.win_dead = deaths.filter(
      (death) => death.player.isRadiant === this.winning_team,
    );
    this.lose_dead = deaths.filter(
      (death) => death.player.isRadiant !== this.winning_team,
    );
    this.during_events = [];
    this.after_events = [];
  ...
  format() {
    ...
    let formatted = [
      renderSentence(template, {
        ...
        win_dead: formatList(
          this.win_dead.map((death) =>
            death.count === 1
              ? PlayerSpan(death.player)
              : [PlayerSpan(death.player), `(x${death.count})`],
          ),
        ),
        lose_dead: formatList(
          this.lose_dead.map((death) =>
            death.count === 1
              ? PlayerSpan(death.player)
              : [PlayerSpan(death.player), `(x${death.count})`],
          ),
    ...
    if (this.during_events.length > 0) {
      formatted = formatted.concat(
        renderSentence(strings.story_during_teamfight, {
    ...
    if (this.after_events.length > 0) {
      formatted = formatted.concat(
        renderSentence(strings.story_after_teamfight, {
...
class TimeMarkerEvent extends StoryEvent {
  ...
  constructor(match: Match, minutes: number) {
    ...
    this.dire_gold = match.players
      ...
      .map((player) => player.gold_t[minutes])
      .reduce((a, b) => a + b, 0);
    this.radiant_percent = Math.round(
      (100 * this.radiant_gold) / (this.radiant_gold + this.dire_gold),
    );
    ...
    return [
      ...
      <StyledStoryNetWorthBar key={`minute_${this.minutes}_networth`}>
        <StyledStoryNetWorthText
          ...
          width={this.radiant_percent}
        />
        <StyledStoryNetWorthText
          color={constants.colorRed}
          width={this.dire_percent}
        />
      </StyledStoryNetWorthBar>,
    ];
  }
...
const generateStory = (match: Match) => {
  ...
  events = events.concat(
    match.teamfights && match.teamfights.length > 0
      ? match.teamfights.map((fight) => new TeamfightEvent(match, fight))
      : [],
  );
  ...
  for (let i = 0; i < events.length; i += 1) {
    if (events[i] instanceof TeamfightEvent) {
      ...
      lastFight !== null &&
      (events[i] instanceof RoshanEvent ||
  ...
  events = events.filter(
    (event, i, list) =>
      i === list.length - 1 ||
      !(
        event instanceof TimeMarkerEvent &&
        list[i + 1] instanceof TimeMarkerEvent
      ),
  );
