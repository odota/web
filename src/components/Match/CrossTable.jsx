  ...
  <Table selectable={false}>
    <TableBody displayRowCheckbox={false}>
      <TableRow>
        ...
        {match.players.slice(0, match.players.length / 2).map((p, i) => (
          <TableRowColumn key={p.hero_id}>
            {heroTd(p, 'hero_id', p.hero_id, i, true)}
          </TableRowColumn>)
        )}
      </TableRow>
      {match.players.slice(match.players.length / 2, match.players.length).map((p, i) => (<TableRow key={p.hero_id}>
        <TableRowColumn>{heroTd(p, 'hero_id', p.hero_id, i, true)}</TableRowColumn>
