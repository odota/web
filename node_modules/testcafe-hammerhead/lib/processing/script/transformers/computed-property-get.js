...
const transformer = {
    ...
    condition: (node, parent) => {
        if (!node.computed || !parent)
            return false;
        if (node.property.type === esotope_hammerhead_1.Syntax.Literal && !instrumented_1.shouldInstrumentProperty(node.property.value))
            return false;
        // super[prop]
    ...
    run: node => node_builder_1.createComputedPropertyGetWrapper(node.property, node.object, node.optional),
};
